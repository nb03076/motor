...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(PWM_sinewave_PI_with_TF_M, 0);
  ...
  PWM_sinewave_PI_with_TF_initialize();
  noInterrupts();
  interrupts();

  /* External Mode initialization */
  errorCode = extmodeInit(PWM_sinewave_PI_with_TF_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(PWM_sinewave_PI_with_TF_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(PWM_sinewave_PI_with_TF_M);
  ...
  /* Terminate model */
  PWM_sinewave_PI_with_TF_terminate();
