...
{
  {
     ...
     */
    if (PWM_sinewave_P.StairGenerator_RepeatPattern >
        PWM_sinewave_P.Switch_Threshold) {
      rtb_MathFunction = PWM_sinewave_P.Constant2_Value;
    } else {
      rtb_MathFunction = PWM_sinewave_P.Constant1_Value;
    }
     ...
     */
    rtb_MathFunction = rt_modd_snf(((PWM_sinewave_M->Timing.clockTick1) * 0.001),
      rtb_MathFunction);
    ...
    for (i = 0; i < 10; i++) {
      rtb_DataTypeConversion[i] = (int8_T)(rtb_MathFunction >=
        PWM_sinewave_P.StairGenerator_TimeInput[i]);
    }
     ...
     *  Store in Global RAM
     */
    DelayInput1_DSTATE = PWM_sinewave_P.DetectFallNonpositive_vinit;
    do {
      rtb_FixPtRelationalOperator = DelayInput1_DSTATE;
      DelayInput1_DSTATE = (rtb_DataTypeConversion[s5_iter - 1] <=
                            PWM_sinewave_P.Constant_Value_o);
      rtb_FixPtRelationalOperator = ((int32_T)DelayInput1_DSTATE > (int32_T)
        rtb_FixPtRelationalOperator);
      if (rtb_FixPtRelationalOperator) {
        rtb_Switch = (int8_T)(s5_iter - PWM_sinewave_P.Constant2_Value_j);
      } else if (rtb_MathFunction >= PWM_sinewave_P.Switch1_Threshold) {
        rtb_Switch = PWM_sinewave_P.Constant1_Value_c;
      } else {
        rtb_Switch = PWM_sinewave_P.Constant3_Value;
      }

      s5_iter++;
    } while ((!rtb_FixPtRelationalOperator) && (s5_iter <= 10));
     ...
     */
    PWM_sinewave_B.DataTypeConversion1 = PWM_sinewave_P.Output_Value[rtb_Switch];
    ...
    rtb_MathFunction = PWM_sinewave_M->Timing.t[0];
    if ((PWM_sinewave_DW.TimeStampA >= rtb_MathFunction) &&
        (PWM_sinewave_DW.TimeStampB >= rtb_MathFunction)) {
      /* Derivative: '<S3>/Derivative' */
      PWM_sinewave_B.Derivative = 0.0;
    } else {
      ...
      if (PWM_sinewave_DW.TimeStampA < PWM_sinewave_DW.TimeStampB) {
        if (PWM_sinewave_DW.TimeStampB < rtb_MathFunction) {
          lastTime = PWM_sinewave_DW.TimeStampB;
          lastU = &PWM_sinewave_DW.LastUAtTimeB;
        }
      } else if (PWM_sinewave_DW.TimeStampA >= rtb_MathFunction) {
        lastTime = PWM_sinewave_DW.TimeStampB;
        lastU = &PWM_sinewave_DW.LastUAtTimeB;
      }

      /* Derivative: '<S3>/Derivative' */
      PWM_sinewave_B.Derivative = (PWM_sinewave_B.Gain3 - *lastU) /
    ...
    if (rtIsNaN(PWM_sinewave_B.Saturation)) {
      rtb_MathFunction = PWM_sinewave_B.Saturation;
    } else if (PWM_sinewave_B.Saturation < 0.0) {
      rtb_MathFunction = -1.0;
    } else {
      rtb_MathFunction = (PWM_sinewave_B.Saturation > 0.0);
    }
     ...
     *  Gain: '<S1>/Gain1'
     */
    rtb_MathFunction = fabs(PWM_sinewave_P.Gain1_Gain *
      PWM_sinewave_B.Saturation);
    ...
    if (PWM_sinewave_M->Timing.t[0] < PWM_sinewave_P.Step_Time) {
      rtb_MathFunction = PWM_sinewave_P.Step_Y0;
    } else {
      rtb_MathFunction = PWM_sinewave_P.Step_YFinal;
    }
