...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) led_contorl_M->Timing.taskTime0;
  led_contorl_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(led_contorl_M, 0);
  ...
  led_contorl_initialize();
  noInterrupts();
  interrupts();

  /* External Mode initialization */
  errorCode = extmodeInit(led_contorl_M->extModeInfo, &rtmGetTFinal
    (led_contorl_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(led_contorl_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(led_contorl_M);
