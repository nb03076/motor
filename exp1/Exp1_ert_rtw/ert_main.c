...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) Exp1_M->Timing.taskTime0;
  Exp1_step();
...
int main(void)
{
  ...
  errorCode = extmodeInit(Exp1_M->extModeInfo, &rtmGetTFinal(Exp1_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Exp1_M, true);
    }
  }
